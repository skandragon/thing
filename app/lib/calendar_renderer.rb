  ...
  def render_xlsx(options, filename)
    ...
    wb.styles do |s|
      ...
      wb.add_worksheet(name: "Pennsic #{Pennsic.year}") do |sheet|
        ...
        @instances.each { |instance|
          instructable = instance.instructable
          start_time = Time.at(instance.start_time.to_f + instance.start_time.utc_offset.to_f + '5:00')
          end_time = Time.at(instance.end_time.to_f + instance.end_time.utc_offset.to_f + '5:00')
          data = [
              instructable.id,
              instance.formatted_location,
              start_time,

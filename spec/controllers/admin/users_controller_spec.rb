...
describe Admin::UsersController, type: :controller do
  ...
  describe 'listing' do
    it "renders user's name, email, and track" do
      ...
      visit admin_users_path
      expect(find('.roles')).to have_content('Admin')
      expect(find('.roles')).to have_content('Coordinator')
      expect(find('.display_name')).to have_content('Fred')
      expect(find('.display_name')).to have_content current_user.email
      current_user.tracks.each { |track|
        expect(find('.tracks')).to have_content track
      }
    end
    ...
    it "renders '-' if the user has no track" do
      ...
      visit admin_users_path
      expect(find('.tracks')).to have_content '-'
    end
  ...
  describe 'edit user' do
    ...
    it 'shows link' do
      visit admin_users_path
      expect(find(@link_id)).to have_content @other_user.email
    end

    it 'link works' do
      visit admin_users_path
      click_on @other_user.display_name
      expect(page).to have_content "Editing #{@other_user.display_name}"
    end
  end
  ...
  describe 'search' do
    ...
    def check_users(role)
      @users.each do |user|
        if user.send(role)
          expect(page).to have_content user.mundane_name
        else
          expect(page).to_not have_content user.mundane_name
        end
      end
    end
    ...
    it 'searches on email' do
      ...
      click_on 'Filter'
      expect(page).to have_content 'mundaneflarg'
      expect(page).to_not have_content 'scabaz'
    end
    ...
    it 'searches on mundane_name' do
      ...
      click_on 'Filter'
      expect(page).to have_content 'scaflarg'
      expect(page).to_not have_content 'scabaz'
      expect(page).to_not have_content 'notateacher'
    end
    ...
    it 'clears' do
      ...
      click_on 'Clear'
      expect(page).to have_content 'mundaneflarg'
      expect(page).to have_content 'scabaz'
    end
  end
end
